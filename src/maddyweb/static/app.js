...
(() => {
  ...
  const closeFloatingMenus = ({restoreFocus = false} = {}) => {
    ...
    state.activeMenuIgnoreScrollUntil = 0;
  ...
  const openFloatingMenu = (menu, {opener = null, row = null, point = null} = {}) => {
    ...
    state.activeMenuIgnoreScrollUntil = window.performance.now() + 250;
  ...
  document.addEventListener("scroll", (event) => {
    ...
    if (
      event.isTrusted
      && window.performance.now() < state.activeMenuIgnoreScrollUntil
    ) return;
    if (
      event.target instanceof Node
