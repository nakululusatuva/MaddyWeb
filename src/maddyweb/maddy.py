...
MAX_ENVELOPE_HEADER_INPUT_CHARS = 4096
MAX_ENVELOPE_HEADER_OUTPUT_CHARS = 512
MAX_ENCODED_WORDS_PER_HEADER = 64
MAX_IMAP_UID = (1 << 32) - 1
...
def _decode_encoded_word(match: re.Match[str]) -> str | None:
    charset, encoding, payload = match.groups()
    if not charset.isascii() or any(not 0x21 <= ord(character) <= 0x7E for character in charset):
        return None
    try:
        if encoding.lower() == "b":
            if _BASE64_WORD_RE.fullmatch(payload) is None or len(payload) % 4 == 1:
                return None
            raw = base64.b64decode(payload + "=" * (-len(payload) % 4), validate=True)
        else:
            raw = _decode_q_word(payload)
            if raw is None:
                return None
...
def _decode_envelope_header(value: str) -> str:
    """Decode bounded RFC 2047 words without rejecting a malformed message."""

    fallback = _clean_envelope_header(value)
    if "=?" not in value or len(value) > MAX_ENVELOPE_HEADER_INPUT_CHARS:
        return fallback

    pieces: list[str] = []
    previous_end = 0
    previous_word_decoded = False
    word_count = 0
    for match in _ENCODED_WORD_RE.finditer(value):
        word_count += 1
        if word_count > MAX_ENCODED_WORDS_PER_HEADER:
            return fallback
        ...
        if decoded is None:
            ...
            previous_word_decoded = True
        previous_end = match.end()
    if word_count == 0:
        return fallback
    pieces.append(value[previous_end:])
    return _clean_envelope_header("".join(pieces))
    ...
    if "- Server meta-data:" not in output:
        ...
        while index < len(lines):
            ...
            record: dict[str, Any] = {
                ...
                "from": _decode_envelope_header(match.group(2)),
                "subject": _decode_envelope_header(match.group(3)),
            }
    ...
    for block in blocks:
        ...
        for line in envelope_text.splitlines():
            ...
            if normalized == "date":
                ...
                record[envelope_keys[normalized]] = (
                    _decode_envelope_header(value)
                    if normalized in _RFC2047_ENVELOPE_FIELDS
                    else value
                )
        uid = record.get("uid")
