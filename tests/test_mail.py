...
def test_html_sanitizer_preserves_passive_table_css_and_drops_active_css() -> None:
    cleaned = sanitize_html_email(
    ...
    for passive_value in (
        ...
        assert passive_value in cleaned

    assert "Position probe" in cleaned
    assert "color:#112233" in cleaned
    for unsafe_value in (
        "position:",
        "background-image",
        "url(",
        ...
        assert unsafe_value not in cleaned
